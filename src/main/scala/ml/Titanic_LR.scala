...
object Titanic_LR {
  def main(args: Array[String]): Unit = {
    //数据处理什么的都是复制Titanic_ml，主要改了一个逻辑回归
    //连接数据源
...
//      .setFeatureSubsetStrategy("auto").setImpurity("gini")
    ...
    val LR = new LogisticRegression().setLabelCol("survived").setPredictionCol("prediction").setFeaturesCol("Features")
    val pipeline = new Pipeline().setStages(Array(titleIndex,sexIndex,cabinIndex,embarked,encoder,vectorAssembler,LR))

    //再加一个交叉验证
    val paraMap = new ParamGridBuilder()
      .addGrid(LR.aggregationDepth,Array(2,3,6,8))
      .build()
    ...
    prediction.show(10)
    prediction.select("passengerId","prediction").write.format("csv").save("src/main/data/resultLR.csv")

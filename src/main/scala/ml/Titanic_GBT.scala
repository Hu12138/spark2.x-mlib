...
import ml.Titanic_ml_concat.getMiddleAge
...
//      .setImpurity("gini").setSeed(3)
    val GBT = new GBTClassifier().setFeaturesCol("Features").setLabelCol("survived").setPredictionCol("prediction").setSeed(3)
    val pipeline = new Pipeline().setStages(Array(titleIndex,sexIndex,cabinIndex,embarked,encoder,vectorAssembler,GBT))
    ...
    val paraMap = new ParamGridBuilder()
      ...
      .addGrid(GBT.maxBins,Array(8,16,32))
      .addGrid(GBT.featureSubsetStrategy,Array("auto","sqrt","log2"))
      .addGrid(GBT.minInstancesPerNode,Array(1,3,10))
    ...
    prediction.select("passengerId","prediction").write.format("csv").save("src/main/data/result13.csv")

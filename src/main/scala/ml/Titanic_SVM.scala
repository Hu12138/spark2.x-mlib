...
//      .setImpurity("gini").setSeed(3)
    //改成SVM
    val SVM = new LinearSVC().setFeaturesCol("Features").setLabelCol("survived").setPredictionCol("prediction")
    val pipeline = new Pipeline().setStages(Array(titleIndex,sexIndex,cabinIndex,embarked,encoder,vectorAssembler,SVM))

    //再加一个交叉验证
    val paraMap = new ParamGridBuilder()
      //.addGrid(SVM.regParam,Array(0,5,10))
      .addGrid(SVM.maxIter,Array(50,100))
      .addGrid(SVM.fitIntercept,Array(true,false))
      //.addGrid(SVM.tol,Array(0,5,10))
      .addGrid(SVM.aggregationDepth,Array(2,4,8))
      .build()
    ...
    prediction.select("passengerId","prediction").write.format("csv").save("src/main/data/resultSVM2.csv")
